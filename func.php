...
function login($post_username)
{
	$postq = json_encode(array(
      'phone_id' => generateUUID(true),
      '_csrftoken' => get_csrftoken(),
      'fb_access_token' => $post_username,
      'guid' => generateUUID(true),
      'device_id' => generateUUID(true),
      'login_attempt_count' => 0
      ));
	$a = instagram(1, generate_useragent(), 'fb/facebook_signup/', 0, generateSignature($postq));
	$header = $a[0];
	$a = json_decode($a[1]);
	if($a->status == 'ok'){
...
function masuk($username)
{
    $login = json_decode(login($username));
...
function directMessage($target,$ua,$cookie,$text){
    ...
    $post = instagram(1, $ua, 'direct_v2/threads/broadcast/text/', $cookie, $data, $headers); 
    $post = json_decode($post[1]);   
    return $post->status;
}

function login2($post_username, $post_password)
{
    $postq = json_encode([
        'phone_id' => generateUUID(true),
        '_csrftoken' => get_csrftoken(),
...
function masuk2($username, $password)
{
    $login = json_decode(login2($username, $password));
    if ($login->result == true) {
        $file = fopen("$username.ig", "w");
        fwrite($file, json_encode(array(
            'cookies' => $login->cookies,
